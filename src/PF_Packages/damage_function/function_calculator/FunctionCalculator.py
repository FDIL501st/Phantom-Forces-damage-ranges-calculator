...
class DamageFunctionCalculator:
    ...
    def __init__(self, damage_info: DmgInfo, max_hp: int) -> None:
    ...
    def calculate_damage_one_hit(self, hit_type: str | Hit, kill_range: float) -> float:
        ...
        # Case between the min and max ranges, so must use damage_drop and calculate the damage
            ...
            # the multi done at the end as that linear equation calculates base damage

            function_result: float = self.__damage_drop * kill_range
            function_result -= self.__damage_drop * self.__min_range
            function_result += self.__d1
            # function_result is done, now multiply the multi
    ...
    def calculate_max_range_hits_kill(self, hits_to_kill: Hits) -> float:
        ...
        kill_range: float = ((self.__MAX_HP / h) - y_int) / self.__damage_drop
        ...
        # f(r) is base damage at range r, assuming damage is always dropping (ignoring how damage is constant before and
        # after min and max range)
        ...
        # If range calculated it outside the valid range the linear equation works in,
        ...
        # Also range calculations can cause some very slightly different numbers due to limitations of representation of
        # numbers
        # within hardware causing super high precision answers to not be accurate (like 10 decimal places)
        # For example, for something where 60 was expected, calculations return 60.0000000000001,
        # so round that to correct it
        return round(kill_range, 3)
